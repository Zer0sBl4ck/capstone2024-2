...
router.post('/usuarios', async (req, res) => {
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash(contrasena, 10); // Usa 'contrasena' aquí

    // Inserta el nuevo usuario en la base de datos
    await db.query(
      `INSERT INTO usuarios (nombre_usuario, correo, contrasena, rol, ubicacion, creado_en, reputacion) 
       VALUES (?, ?, ?, 'normal', ?, NOW(), 0)`, // Cambia 'contraseña' por 'contrasena'
      [nombre_usuario, correo, hashedPassword, ubicacion] // Usa 'hashedPassword' aquí
    );
